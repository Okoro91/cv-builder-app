...
const CustomSectionForm = ({
  ...
  // Reset form when section changes or form is closed
  useEffect(() => {
    if (!showSectionForm) {
  ...
  // Handle editing existing section
  useEffect(() => {
    if (editingId && sections) {
  ...
  const handleSubmitSection = (e) => {
    e.preventDefault();
    if (editingId) {
      onUpdate(editingId, sectionForm);
    } else {
      onAdd(sectionForm);
    }
    setSectionForm({ name: "", description: "" });
  ...
  const handleSubmitItem = (e) => {
    e.preventDefault();
    if (editingItemId) {
      onUpdateItem(sections.id, editingItemId, itemForm);
    } else {
      onAddItem(sections.id, itemForm);
    }
    setItemForm({
      title: "",
      subtitle: "",
  ...
  const handleRemoveSection = () => {
    if (
      window.confirm(
        `Are you sure you want to remove "${sections?.name}" sections?`,
      )
    ) {
      onRemove(sections.id);
      onCancelEdit();
    }
  };

  // ALWAYS show the section creation form at the top
  return (
    <div className="bg-white p-6 rounded-lg shadow-md mb-6">
      {/* Section Creation Form - ALWAYS VISIBLE */}
      <div className="mb-6">
      ...
      {/* Selected Section Content - Only show if a section is selected */}
      {sections && (
        <div className="border-t pt-6 mt-2">
          {sections.map((section) => (
            <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        value={itemForm.date}
                        onChange={(e) =>
                          handleItemChange("date", e.target.value)
                        }
                        className="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              {sections.items && section.items.length > 0 ? (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex space-x-2 ml-4">
                        ...
                        <button
                          onClick={() => onRemoveItem(sections.id, item.id)}
        ...
        <div className="border-t pt-6 mt-2">
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm text-gray-400">
              Create a new section above and click "Edit" button to manage its
              content
            </p>
          </div>
        </div>
